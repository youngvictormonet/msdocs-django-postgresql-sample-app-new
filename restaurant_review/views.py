...
def users_uplodad():
    ...
    df = pd.DataFrame({'Twitter' : [all_users[id].twitter for id in range(len(all_users)-5,len(all_users))],
                             'Points': [all_users[id].points for id in range(len(all_users)-5,len(all_users))]},
                      index = [all_users[id].date_created.strftime("%H:%M:%S")  for id in range(len(all_users)-5,len(all_users))])
